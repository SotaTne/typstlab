...
fn test_counter_without_lock_exposes_race() {
    // Verify counter helper WITHOUT locking has lost updates
    ...
    // Spawn processes without locking
    let mut handles = vec![];
    for _ in 0..NUM_PROCESSES {
        ...
        let handle = std::thread::spawn(move || {
            let status = Command::new(example_bin("counter_child"))
                ...
                .status()
                .expect("Failed to execute counter_child");

            assert!(status.success(), "counter_child should exit successfully");
        });
        handles.push(handle);
    }
    ...
    // Verify: WITHOUT locks, final count < expected (lost updates)
    ...
    assert!(
        ...
        "Without locks, should have lost updates: got {}, expected < {}",
        final_count,
        expected
    );
}
